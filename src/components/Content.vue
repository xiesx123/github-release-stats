<template>
  <div class="content-root">
    <el-container class="container">
      <el-header height="140px" class="query-header">
        ...
        <div class="repo-right">
          <el-button
            ...
            @click="startAnalysis"
            plain
            ><i :class="beginStatusClass"></i>Analysis</el-button
          >
        </div>
        <div class="repo-left">
          <el-input
        ...
        <div class="common-info-container" v-if="totalDownloads">
          ...
          <el-alert :closable="false" type="success" id="top-total">
            <i class="el-icon-download"
              >Total Downloads: <b>{{ totalDownloads }}</b></i
            >
          </el-alert>

          <el-button size="mini" type="info" plain @click="toggleChart"
            >Toggle Chart Type</el-button
          >
          <el-button
            size="mini"
            type="info"
            plain
            @click="setPatternDialog = true"
            >Exclude Asset File
            <span style="color: red;">{{ filterPattern }}</span></el-button
          >
          <ve-chart
            :legend-visible="isPieChart"
            :data="chartData"
          ...
          <el-card>
            <el-table
              v-if="downloadsByReleases.length != 0"
            ...
            >
              ...
              <el-table-column sortable prop="count" label="Download">
              </el-table-column>
              <el-table-column
                sortable
                prop="percent"
            ...
            >
              <el-table-column prop="name" label="Assets">
                <template slot-scope="scope">
                  ...
                  <a
                    :href="scope.row.browser_download_url"
                    style="margin-left: 10px"
                    >{{ scope.row.name }}</a
                  >
                </template>
              </el-table-column>
              <el-table-column prop="download_count" label="Download">
              </el-table-column>
            </el-table>
          </div>
        </el-card>
...
export default {
  ...
  computed: {
    ...
    downloadsByReleases() {
      ...
      for (const releaseData of download) {
        releaseData.percent = (
          (releaseData.count / total).toFixed(4) * 100
        ).toFixed(2);
      }
  ...
  methods: {
    syncQueryParams(extra = {}) {
      ...
      const repo = (extra.repo !== undefined
        ? extra.repo
        : this.repoURL || ""
      ).trim();
      const pattern = (extra.pattern !== undefined
        ? extra.pattern
        : this.filterPattern || ""
      ).trim();
    ...
    startAnalysis() {
      ...
      let releaseURL =
        `https://api.github.com/repos/${info.owner}/${
          info.name
        }/releases?ran=` + Math.random();
    ...
    filterFile(data) {
      ...
      for (const release of data) {
        for (
          var assetIndex = 0;
          assetIndex < release.assets.length;
          assetIndex++
        ) {
    ...
    getfilterPattern() {
      ...
      const regexParts = patterns.map((p) => {
        // 先把 * 替换成一个不常用占位符，然后转义其他正则特殊字符，最后把占位符替换成 .*
        return p
          .replace(/\*/g, "__STAR__")
          .replace(/[.*+?^${}()|[\]\\]/g, "\\$&")
          .replace(/__STAR__/g, ".*");
